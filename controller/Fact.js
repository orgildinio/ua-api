...
exports.createFact = asyncHandler(async (req, res, next) => {
  ...
  flds.map((el) => delete req.body[el]);
...
exports.getFacts = asyncHandler(async (req, res) => {
  ...
  const stringLanguageFld = ["name", "about"];
  ...
  const pagination = await paginate(page, limit, Fact, result);
  query.limit(limit);
...
exports.getFact = asyncHandler(async (req, res) => {
...
exports.updateFact = asyncHandler(async (req, res) => {
...
exports.multDeleteFact = asyncHandler(async (req, res) => {
  const ids = req.queryPolluted.id;
  const finds = await Fact.find({ _id: { $in: ids } });

  if (finds.length <= 0) {
    throw new MyError("Таны сонгосон өгөгдөлүүд байхгүй байна", 400);
  }

  const facts = await Fact.deleteMany({ _id: { $in: ids } });
